...
var adresIN = ["data", "agad", "json"];
var adresOUT = ["data", "roboczy"];
var przetworzonyJSON = "roboczy01.json"
// ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||

ZapiszJSON(path.resolve(__dirname, adresOUT[0], adresOUT[1], przetworzonyJSON), Konwertuj_Pliki(path.resolve(__dirname, adresIN[0], adresIN[1], adresIN[2])));
...
function Lokalizacja(polozenie, Sygnatury, plikZrodlowy) {
  this.polozenie = polozenie;
  this.Sygnatury = Sygnatury;
  this.plikZrodlowy = plikZrodlowy;
}
function MetryczkaZESPOLU(urlZespolu, tytulZespolu, podtytulZespolu, autorZespolu, publikacjaZespolu, plikZrodlowy) {
  this.urlZespolu = urlZespolu;
  this.tytulZespolu = tytulZespolu;
  this.podtytulZespolu = podtytulZespolu;
  this.autorZespolu = autorZespolu;
  this.publikacjaZespolu = publikacjaZespolu;
  this.publikatorZespolu = "Archiwum Główne Akt Dawnych w Warszawie";
  this.plikZrodlowy = plikZrodlowy;
}
function RekordINWENTARZA(nrZespolu, tytul, zasob, sygnatura, sygnaturaDawna, opis, daty, info, jenzyki, skany, mikrofilm, uwagi, plikZrodlowy) {
  this.tytul = tytul;
  this.zasob = zasob;
  this.nrZespolu = nrZespolu;
  this.nic = "-------------------";
  this.sygnatura = sygnatura;
  this.sygnaturaDawna = sygnaturaDawna;
  this.opis = opis;
  this.daty = daty;
  this.info = info;
  this.jenzyki = jenzyki;
  this.skany = skany;
  this.mikrofilm = mikrofilm;
  this.uwagi = uwagi;
  this.plikZrodlowy = plikZrodlowy;
}
