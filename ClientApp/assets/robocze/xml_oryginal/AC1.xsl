...
<!-- Wersja PL
EADCB6.xsl przemianowany na AC1.xsl 
dla potrzeb zespołów/zbiorów mających wyodrębnione serie: 
c01 - seria
c02 - jednostki archiwalne 
c03 - wpis/dokumenty
do arkusza jest dołączony  dsc14.xsl
HWs 06.06.08 -->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="/ead">
		<html>
			<head>
				<style type="text/css">
				h1, h2, h3, h4 {font-family: Verdane; font-size: 12pt}
				...
				<title>
					...
					<xsl:value-of select="eadheader/filedesc/titlestmt/titleproper/author"/>
	...
	two columns. -->
			<body>
				...
				<center>
					<img src="Orzel.gif" WIDTH="90" HEIGHT="60" BORDER="0"></img>
				</center>
...
a matching element with content in the finding aid.-->
		...
		or string(archdesc/*/accessrestrict/head)">
			<p style="margin-top:-5pt">
				<b>
					<a href="#restrictlink">
						<xsl:text>Udostępnianie</xsl:text>
					</a>
				</b>
			</p>
		</xsl:if>
			...
			<p style="margin-top:-5pt">
				<b>
					<a href="#relatedmatlink">
						<xsl:text>Materiały Pokrewne</xsl:text>
					</a>
				</b>
			</p>
		</xsl:if>
		...
		or string(archdesc/*/accruals/*)">
			<p style="margin-top:-5pt">
				<b>
					<a href="#adminlink">
					<xsl:text>Pozostałe Informacje </xsl:text>
					</a>
				</b>
			</p>
		</xsl:if>
	...
	<!--This template converts a Extref element into an HTML anchor (HWs).   -->
	
	<!-- POOPRAW -->

<xsl:template match="extref">
<xsl:variable name="target1">
<xsl:value-of select="@target1"/>
</xsl:variable>
<a href="{@href}" target1="_self">
<xsl:value-of select="."/>
</a>
</xsl:template> 
		...
		string(archdesc/*/separatedmaterial)">
			<h3>
				<a name="relatedmatlink">
					<b>
						<xsl:text>Inne Materiały Pokrewne
</xsl:text>
					</b>
				</a>
			</h3>
	...
	<xsl:template name="archdesc-restrict">
		...
		or string(archdesc/*/accessrestrict/*)">
			<h3>
				<a name="restrictlink">
					<b>
						<xsl:text>Udostępnianie - Ograniczenia</xsl:text>
					</b>
				</a>
			</h3>
